...
def set_node_properties(labels, **kwargs):
    ...
    if center is not None:
        ...
        if logger is not None:
            logger.info("Set the center state at: [%s]" % (center))
...
def set_edge_properties(df, **kwargs):
    ...
    # Compute delta
    if (
        ~np.any(df.columns == "delta")
        and isinstance(df, pd.DataFrame)
        and np.any(df.columns == state)
        and np.any(df.columns == datetime)
        and np.any(df.columns == sample_id)
    ):
        if logger is not None:
            logger.info("Standardizing input dataframe using method: [%s]." % (method))
        ...
        df = standardize(
            ...
            datetime=datetime,
            dt_format=dt_format,
            minimum_time=timedelta,
            logger=logger,
        )
    else:
        raise Exception(
            print(
                'Can not find the specified columns: "state", "datetime", or "sample_id" columns in the input dataframe: %s'
                % (df.columns.values)
            )
        )
...
def _set_nodecolor(df, sample_id, color, cmap, logger):
    ...
    if not np.any(np.isin(df.columns, "color")):
        df["color"] = color
        if logger is not None:
            logger.info("Set all nodes to color: %s" % (color))
...
def _set_nodesize(df, sample_id, size, logger):
    ...
    if not np.any(np.isin(df.columns, "size")):
        df["size"] = size
        if logger is not None:
            logger.info("Set all nodes to size: %d" % (size))

    return df


def first_items(df, column):
    sampleid = "sample_id"
    return (
        df[sampleid, column]
        .drop_duplicates(sampleid)
        .set_index(sampleid)[column]
        .to_dict()
    )
...
def show(df, **kwargs):
    ...
    X = []
    sid = np.array(
        list(map(lambda x: labels.get(x)["id"], df[config["columns"]["state"]].values))
    )
    ...
    act_counts = dict(
        zip(df_labels["id"].astype(str), np.zeros(len(df_labels["id"])).astype(int))
    )
    ...
    if config["note"] is None:
        config[
            "note"
        ] = "This is a simulation of multiple states and samples. <a href='https://github.com/d3blocks/d3blocks'>d3blocks movingbubbles</a>."
        config["note"] = (
            config["note"]
            + "\nDate start: "
            + str(datestart)
            ...
            + str(datestop)
            + "\nRuntime: "
            + str(datestop - datestart)
            + "\nEstimated time to Finish: "
            + str(datestart + (datestop - datestart))
        )
...
def write_html(X, config, logger=None):
    ...
    content = {
        ...
        "START_TIME": zero_to_hour
        + str(config["start_hour"])
        + ":"
        + zero_to_min
        + str(config["start_minute"]),
    ...
    try:
        ...
        jinja_env = Environment(
            loader=PackageLoader(
                package_name="d3blocks.movingbubbles", package_path="d3js"
            )
        )
...
def standardize(
    ...
    if method == "samplewise":
        ...
        for sid in uisample_id:
            getidx.append(
                df[[sample_id, datetime]]
                .loc[df[sample_id] == sid]
...
def generate_data_with_random_datetime(
    n=10000,
    c=1000,
    date_start=None,
    date_stop=None,
    ...
    df = pd.DataFrame(
        columns=["datetime", "sample_id", "state"],
        data=np.array([[None, None, None]] * n),
    )
    location_types = [
        "Home",
        "Hospital",
    ...
    while i <= df.shape[0] - 3:
        ...
        df["datetime"].iloc[i] = random_date(
        ...
        df["datetime"].iloc[i] = random_date(
            df["datetime"].iloc[i - 1], date_stop, random.random(), dt_format=dt_format
        )
        i = i + 1
